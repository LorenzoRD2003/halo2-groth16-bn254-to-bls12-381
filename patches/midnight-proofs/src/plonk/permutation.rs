...
/// reconstructs the derived coefficient/coset forms lazily when promoted to a
/// full [`ProvingKey`].
#[derive(Clone, Debug)]
pub struct BaseProvingKey<F: PrimeField> {
    pub(crate) permutations: Vec<Polynomial<F, LagrangeCoeff>>,
}

impl<F: WithSmallOrderMulGroup<3> + SerdeObject> BaseProvingKey<F> {
    /// Reads a lean permutation setup artifact from a buffer.
    pub fn read<R: io::Read>(reader: &mut R, format: SerdeFormat) -> io::Result<Self> {
        let permutations = read_polynomial_vec(reader, format)?;
        Ok(BaseProvingKey { permutations })
    }

    /// Writes a lean permutation setup artifact to a buffer.
...
impl<F: WithSmallOrderMulGroup<3>> BaseProvingKey<F> {
    /// Promotes the lean permutation setup artifact into a full proving key.
    pub(crate) fn finalize(self, domain: &EvaluationDomain<F>, p: &Argument) -> ProvingKey<F> {
...
impl<F: WithSmallOrderMulGroup<3> + SerdeObject> ProvingKey<F> {
    ...
    pub(super) fn read<R: io::Read>(
        ...
        Ok(BaseProvingKey::read(reader, format)?.finalize(domain, p))
    }
    ...
    pub(super) fn write<W: io::Write>(&self, writer: &mut W) -> io::Result<()> {
        BaseProvingKey { permutations: self.permutations.clone() }.write(writer)
    }
}
