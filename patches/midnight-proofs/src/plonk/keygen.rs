...
use super::{
    ...
    BaseProvingKey, permutation, Challenge, Error, LagrangeCoeff,
    Polynomial, ProvingKey, VerifyingKey,
};
use crate::{
    circuit::Value,
...
pub fn k_from_circuit<F: Ord + Field + FromUniformBytes<64>, C: Circuit<F>>(circuit: &C) -> u32 {
    cost_model_options(circuit).min_k
}
...
where
    ...
    ConcreteCircuit: Circuit<F>,
{
    Ok(keygen_pk_base(vk, circuit)?.finalize())
}

/// Generate a lean proving-state setup artifact from a `VerifyingKey` and an
/// instance of `Circuit`.
pub fn keygen_pk_base<F, CS, ConcreteCircuit>(
    vk: VerifyingKey<F, CS>,
    circuit: &ConcreteCircuit,
) -> Result<BaseProvingKey<F, CS>, Error>
where
    F: WithSmallOrderMulGroup<3> + FromUniformBytes<64>,
    CS: PolynomialCommitmentScheme<F>,
    ConcreteCircuit: Circuit<F>,
{
    ...
    let permutation_pk = assembly.permutation.build_base_pk::<F>(&vk.domain, &cs.permutation);
...
/// Computes the common extended-Lagrange basis polynomials used by the prover
/// from an already materialized verification key and constraint system.
pub fn compute_lagrange_polys<F, CS>(
    vk: &VerifyingKey<F, CS>,
