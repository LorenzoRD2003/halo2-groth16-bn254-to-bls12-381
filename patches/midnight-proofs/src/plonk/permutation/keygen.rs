...
use super::{Argument, BaseProvingKey, ProvingKey, VerifyingKey};
use crate::{
...
impl Assembly {
    ...
    pub(crate) fn build_pk<F: WithSmallOrderMulGroup<3>>(
        ...
        build_base_pk::<_>(domain, p, |i, j| self.mapping[i][j]).finalize(domain, p)
    }

    pub(crate) fn build_base_pk<F: WithSmallOrderMulGroup<3>>(
        self,
        domain: &EvaluationDomain<F>,
        p: &Argument,
    ) -> BaseProvingKey<F> {
        let _ = p;
        build_base_pk::<_>(domain, p, |i, j| self.mapping[i][j])
    }
