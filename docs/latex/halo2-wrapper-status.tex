...
\(\downarrow\)\\
\texttt{MidnightDirectOuterBackendBls12Host}\\
\(\downarrow\)\\
\texttt{setup/prove/verify}
\end{quote}

La pol\'itica actual de lanes es:

\begin{itemize}
  \item \texttt{BLS12-381} es la lane oficial del outer;
  \item \texttt{BN254} se mantiene como lane de compatibilidad, testing y comparaci\'on;
  \item la sem\'antica del statement outer y el binding del VK son compartidos entre ambas lanes.
\end{itemize}
...
La superficie directa concreta hoy expone dos lanes reales:

\begin{itemize}
  \item \texttt{MidnightDirectOuterBackendBls12Host};
  \item \texttt{MidnightDirectOuterBackendBn254Host}.
\end{itemize}

La primera es la lane oficial del outer.
La segunda se retiene para compatibilidad, testing y regresiones comparativas.
