...
use wrapper_circuits::{
  ...
  OuterStatementSemantics, OuterWrapperCircuit, OuterWrapperCircuitInput, build_outer_wrapper_circuit,
};
use wrapper_core::{
  ExpectedWrapperArtifacts, OuterStatementContractError, ProofSystemKind, ProducedOuterGroth16ArtifactBundle,
  ProducedOuterGroth16ProofJson, ProducedOuterGroth16VerificationKeyJson, WrapperExecutionPackage,
};
...
pub enum OuterGroth16BackendError {
  ...
  #[error(
    "outer backend expected target proof system '{expected}', got '{actual}'"
  )]
  UnsupportedTarget {
  ...
  #[error("failed to parse inner proof payload for source artifact '{source_artifact_id}': {source}")]
  MalformedInnerProof {
    /// Source artifact identifier from the package witness metadata.
    source_artifact_id: String,
    #[source]
  ...
  #[error("failed to parse inner verification-key payload for source artifact '{source_artifact_id}': {source}")]
  ...
  #[error(
    "outer setup verification-key protocol mismatch: expected '{expected}', got '{actual}'"
  )]
  ...
  #[error(
    "outer setup verification-key nPublic mismatch: expected {expected}, got {actual}"
  )]
  ...
  #[error(
    "outer setup verification-key IC length mismatch: expected {expected}, got {actual}"
  )]
  ...
  #[error(
    "outer produced proof top-level key mismatch: expected {expected:?}, got {actual:?}"
  )]
  ...
  #[error("outer backend '{backend}' does not implement '{operation}' in the current repository phase")]
...
const ARKWORKS_BACKEND_METADATA: OuterGroth16BackendMetadata = OuterGroth16BackendMetadata {
  ...
  stack: "halo2/midnight outer wrapper lane targeting Groth16 BLS12-381 artifacts",
  protocol: "groth16",
  curve: "bls12-381",
  setup_assumptions: &[
    "the outer circuit is authored in halo2/midnight and is the canonical circuit surface",
    "the concrete outer backend will use one Groth16 CRS per outer circuit configuration",
    "trusted setup output must be serialized once and then reused across proofs for that configuration",
  ...
  compatibility_notes: &[
    "the current backend adapter targets a halo2/midnight outer circuit, not a second circuit implementation in another stack",
    "artifact shapes remain aligned with the current wrapper-core expected output model",
    "steps 5-8 still need witness adaptation, setup materialization, proof generation, and verification wiring",
  ],
};
...
fn parse_native_input_value(
  ...
  if let Some(hex) = value.strip_prefix("0x") {
    ...
    for ch in hex.chars() {
      let digit = ch.to_digit(16).ok_or_else(|| OuterGroth16BackendError::InvalidPublicInputValue {
        context,
  ...
  NativeField::from_str_vartime(value).ok_or_else(|| OuterGroth16BackendError::InvalidPublicInputValue {
    context,
    field_name: field_name.to_owned(),
    value: value.to_owned(),
  })
...
impl OuterGroth16Backend for ArkworksGroth16Bls12381Backend {
  ...
  fn prepare(
    ...
    planned.notes.push(format!(
      "selected outer backend stack: {}",
      self.metadata().stack
    ));
    planned.notes.extend(
      self
        .metadata()
        .serialization_conventions
        .iter()
    ...
    planned.bundle_template.notes.push(
      "selected backend is the arkworks Groth16 BLS12-381 lane; proof generation remains pending".to_owned(),
    );
  ...
  fn prove(
    ...
    let _ = self.prepare(package)?;
    Err(OuterGroth16BackendError::MissingOuterProofBackend {
      circuit_stack: "halo2/midnight outer wrapper circuit",
      prover_kind: "Groth16 BLS12-381",
    })
  }
...
impl ArkworksGroth16Bls12381Backend {
  ...
  pub fn adapt_input(
    ...
    let proof_json = artifacts.proof_json.ok_or_else(|| OuterGroth16BackendError::MissingInnerProofPayload {
  ...
  pub fn build_outer_circuit(
    ...
    circuit
      ...
      .map_err(|error| OuterGroth16BackendError::OuterCircuitInputInvalid {
...
mod tests {
  ...
  fn planned_bls12_381_backend_prepares_placeholder_bundle() {
    ...
    let verification_key = bundle
      .verification_key
      .as_ref()
      .expect("planned backend should materialize a VK skeleton");
  ...
  fn arkworks_backend_exposes_selected_stack_metadata() {
    ...
    assert_eq!(
      metadata.stack,
      "halo2/midnight outer wrapper lane targeting Groth16 BLS12-381 artifacts"
    );
  ...
  fn arkworks_backend_prepares_expected_output_with_stack_note() {
    ...
    assert!(planned
      ...
      .notes
      .iter()
      .any(|note| note.contains("arkworks Groth16 BLS12-381 lane")));
  }
  ...
  #[test]
  fn arkworks_backend_reports_missing_outer_prover_backend() {
    let backend = ArkworksGroth16Bls12381Backend;
    let package = sample_package(ProofSystemKind::Groth16Bls12_381);
  ...
  fn arkworks_backend_rejects_setup_verification_key_with_wrong_ic_length() {
    ...
    assert!(matches!(
      backend.validate_setup_verification_key(&package, &vk),
      Err(OuterGroth16BackendError::VerificationKeyShapeMismatch {
        expected: 3,
        actual: 2,
      })
    ));
  ...
  fn arkworks_backend_adapts_real_fixture_inputs() {
    ...
    assert_eq!(
      adapted.outer_statement.field_names,
  ...
  fn arkworks_adapted_input_can_map_to_circuit_owned_input() {
    ...
    assert_eq!(
      circuit_input.outer_statement.public_inputs,
      adapted.outer_statement.public_inputs
    );
  }
  ...
  fn arkworks_adapter_rejects_verification_key_arity_mismatch_against_package() {
    ...
    let ic = vk_json["IC"]
      .as_array_mut()
      .expect("fixture VK should carry an IC array");
    ic.push(ic[0].clone());
    ...
    assert!(matches!(
      ...
      Err(OuterGroth16BackendError::VerificationKeyIcCountMismatch {
        expected: 2,
        actual: 3,
      })
    ));
