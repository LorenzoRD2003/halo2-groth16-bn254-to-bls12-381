...
- G2 subgroup checks or scalar multiplication, or Fp12 support
...
- Implementing full G2 arithmetic, Fp12, pairings, or Groth16 verification
- Selecting a final proving backend
- Claiming compatibility with production proof artifacts

## Disclaimer

This repository now contains a circuit-backed BN254 primitive layer using `midnight-circuits` and `midnight-proofs`, organized under `wrapper-circuits/src/bn254/`, including a first Week 2 Fp2 slice, a minimal Fp6 slice, a minimal G2 affine slice, and a narrow Jacobian G2 projective slice. The Fp6 layer currently supports `add`, `sub`, `neg`, `mul`, and `square` over the arkworks-compatible tower; the projective G2 layer supports non-identity `from_affine`, `neg`, `double`, and incomplete `add`. The repository still does not include subgroup checks, scalar multiplication, Fp12, pairings, Groth16 verification, or a wrapper verifier circuit. Current Criterion benchmarks are sanity-check hooks over small implemented circuits and should not be read as production cryptographic performance claims.
